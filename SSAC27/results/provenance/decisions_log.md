...
`SSAC27/results/task*/` and `SSAC27/data/processed/`.

**Why.** The repository convention is that each conference project installs,
runs, and validates entirely within its own directory. SSAC scripts read only
`SSAC27/data/` and write only `SSAC27/results/`.

---

## D1 — The raw constituent statistics exist; the stated limitation was stale

Earlier SaberSeminar documentation, now preserved in Git history, said that the
raw-statistic Ridge control could not be run.

**Finding.** `data/raw/batting_stats_2021_2025.csv` (2,309 rows x 462 columns,
recovered from the 2026-08-12 FanGraphs export)
...
**Join integrity check, run on every execution** (`common.load_player_seasons`,
...
PA match, and is mapped in `common.NAME_ALIASES`:
...
(`results/task1/ubr_robustness.csv`). Since `BsR = UBR + wSB + wGDP`, most of the
...
**Adopted.** `scripts/data.py` splits what had been one population into two:
...
They remain in `common.py` because Tasks 1-3 still read the archived
name-keyed table; new code should use `data.py`.
...
structurally unfavourable test of the tool structure. WAR prices both. The SaberSeminar result
`../SaberSeminar26/results/delta_association_coefficients.csv` reports the matching
