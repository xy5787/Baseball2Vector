...
Run:  python SSAC27/scripts/task4_comparable_players.py
"""
...
def main() -> None:
    (sc.RESULTS_DIR / "task4").mkdir(parents=True, exist_ok=True)
    ...
    summary.to_csv(sc.RESULTS_DIR / "task4" / "retrieval_performance.csv", index=False)
    paired.to_csv(sc.RESULTS_DIR / "task4" / "paired_differences.csv", index=False)
    pd.concat(prediction_frames, ignore_index=True).to_csv(
        sc.RESULTS_DIR / "task4" / "predictions.csv", index=False
    )
    (sc.RESULTS_DIR / "task4" / "environment.json").write_text(
        json.dumps(
