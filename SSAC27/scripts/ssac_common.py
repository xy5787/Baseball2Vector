...
Design contract (see ``SSAC27/results/decisions_log.md`` for the reasoning):
...
INTERMEDIATE_DIR = SSAC_ROOT / "data_intermediate"
...
# Player-season table
# --------------------------------------------------------------------------- #
def _archived_player_seasons() -> pd.DataFrame:
    """Archived tool scores, targets and stable player IDs (matched rows only)."""
    df = pd.read_csv(ARCHIVED_IDS_PATH)
    ...
    # Forecast-safe re-standardization: the archived 20-80 columns are a single
    # pooled affine map of the within-season tool score, so standardizing inside
    # the input season exactly undoes the pooled step. No future season is read.
    for prefix in ENCODINGS.values():
        for tool in TOOL_NAMES:
            source = f"{prefix}_{tool}"
            matched[f"safe_{source}"] = matched.groupby("Season")[source].transform(
                lambda x: (x - x.mean()) / x.std(ddof=0)
            )
    return matched


def _constituent_features() -> pd.DataFrame:
    """Direction-adjusted, within-input-season z-scored constituent statistics.

    This deliberately applies the *same* transform chain the five-tool scores are
    built from (Bayesian stabilization -> within-season z-score -> direction
    flip), so that the 5 tool scores are exactly the five group means of these
    22 columns. M1 is then a fixed linear projection of the M2 feature space, and
    M1-vs-M2 isolates the effect of the tool grouping alone.
    """
    raw = pd.read_csv(RAW_EXPORT_PATH)
    processed = preprocess(raw.copy())
    groups = build_final_groups(processed)
    stats = [c for cols in groups.values() for c in cols]
    assert stats == CONSTITUENT_STATS, (stats, CONSTITUENT_STATS)

    aligned = apply_direction(season_zscore(processed, stats))
    out = aligned.rename(columns=dict(zip(CONSTITUENT_STATS, CONSTITUENT_COLS)))
    out["Season"] = processed["Season"].to_numpy()
    out["join_key"] = (
        processed["Name"].map(normalize_name).map(lambda s: NAME_ALIASES.get(s, s))
    )

    # Recomputed z-score tool scores, kept for the join-integrity check below.
    recomputed = zscore_tool_scores(aligned, groups)
    for tool in TOOL_NAMES:
        out[f"recomputed_{tool}"] = recomputed[tool].to_numpy()
    return out


def load_player_seasons(verbose: bool = True) -> pd.DataFrame:
    """Archived tool scores + targets joined to raw constituent statistics."""
    archived = _archived_player_seasons()
    archived["join_key"] = archived["Name"].map(normalize_name)
    constituents = _constituent_features()

    assert not constituents.duplicated(["join_key", "Season"]).any()
    merged = archived.merge(
        constituents, on=["join_key", "Season"], how="left", validate="many_to_one"
    )
    unjoined = merged[CONSTITUENT_COLS[0]].isna().sum()
    assert unjoined == 0, f"{unjoined} archived rows had no raw-export match"

    if verbose:
        # Join integrity: recomputed tool scores must track the archived ones.
        for tool in TOOL_NAMES:
            a = merged.groupby("Season")[f"safe_ZScore_{tool}"].transform(lambda x: x)
            b = merged.groupby("Season")[f"recomputed_{tool}"].transform(
                lambda x: (x - x.mean()) / x.std(ddof=0)
            )
            r = float(np.corrcoef(a, b)[0, 1])
            assert r > 0.99, f"{tool} join-integrity correlation {r:.4f} too low"
            print(f"  join check {tool:<11s} archived vs recomputed r = {r:.5f}")

    return add_rate_columns(merged).sort_values(
...
def environment_stamp() -> dict:
    ...
    return {
        ...
        "raw_export_sha256": sha256_file(RAW_EXPORT_PATH),
        "archived_ids_sha256": sha256_file(ARCHIVED_IDS_PATH),
    }
