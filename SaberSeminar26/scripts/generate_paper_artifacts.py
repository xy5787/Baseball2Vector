...
from baseball2vec.repro_common import N_BOOT, REPO_ROOT, RUN_ROOT, SEED, TOOL_COLS, percentile_ci, sha256_file


RESULTS = RUN_ROOT / "results"
FIGURES = RUN_ROOT / "paper" / "figures"
TABLES = RUN_ROOT / "paper" / "tables"
...
def figure1() -> dict[str, float]:
    frame = pd.read_csv(RUN_ROOT / "data" / "processed" / "player_seasons.csv")
...
def delta_coefficients() -> pd.DataFrame:
    transitions = pd.read_csv(RUN_ROOT / "data" / "processed" / "transitions.csv")
...
def reproduction_check(alignment: dict[str, float]) -> None:
    ...
    checks = {
        "canonical_manuscript_sha256": sha256_file(REPO_ROOT / "paper" / "baseball2vector_en.tex"),
