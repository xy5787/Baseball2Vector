"""SD10Pipeline version of scripts/04_roi_analysis.py: unchanged regression
logic, but reads/writes within this isolated folder, operating on the
mean=50/SD=10-scaled v3_results.csv produced by 03_joint_vae.py here.

Note: IMPROVEMENT_STEP=5.0 (a synthetic +5-point tool improvement) is more
consistent under this scale than under the old MinMax one -- 5 points now
means the same thing (+0.5 SD) for every tool, whereas under MinMax it was
an arbitrary fraction of each tool's own sample range.
"""
...
from baseball2vec.roi import TOOL_NAMES, build_delta_dataset, compute_individual_roi, run_regression
...
print("=" * 60)
print("SD10Pipeline Step 4: ROI analysis")
print("=" * 60)
...
plot_roi_coefficients(wrc_res, figures_dir=FIG_DIR)
...
plot_partial_dependence(wrc_res.rf10_model, X_full, TOOL_NAMES, figures_dir=FIG_DIR)

plot_delta_scatter(delta_clean, TOOL_NAMES, figures_dir=FIG_DIR)

prospect_df = compute_individual_roi(player_df, wrc_res.rf10_model)
latest_season = int(player_df["Season"].max())
plot_roi_individual(prospect_df, TOOL_NAMES, latest_season, figures_dir=FIG_DIR)
