# Arm D — Blind vector reasoning with subject-excluded tools (v2)
...
The comparable pool excludes every historical season belonging to the anonymous subject. Treat an unavailable lookup as unavailable data, not as evidence for reverse-identification.
Available tools:
...
- `find_comparables(vec5, k)`: nearest historical player-season IDs. Pass
  **Vector at t exactly**; never pass the observed delta, whose values are not
  current 20–80 tool levels.
